...
formsName=formsName+document.forms[i].name+",";
}else
{
formsName=formsName+document.forms[i].name;
}
...
/*Insert the Report Suite ID where you wish to store data*/
omtr_omnitureRSID="citiintlindiaprod";

/*Insert the top level domain(s) that should be tracked as internal or part of the site itself. 
* Leave out http/https & subdomains (e.g. citi.com NOT http://www.citi.com */
omtr_internalDomain="javascript:,citibank.co.in,citinri.com";

/*Insert country - could be TR for Turkey or IPB*/
omtr_countryID="IN";

/*Insert the friendly site name - e.g. CitiBank Turkey */
omtr_siteID="CitiBank India";

/*Insert external campaign parameter used in query string - use comma as a delimiter if more than one*/
omtr_externalcampaignID="cid";

/*Insert internal campaign parameter used in query string - use comma as a delimiter if more than one*/
omtr_internalcampaignID="eOfferCode";

/*Insert the charset using on the website*/
omtr_charSet="UTF-8";

/*Insert timezone. If CET insert +1 - other examples 0 or -6*/
omtr_timezone="+5";
...
/****Other milestones: START****/

/*Put the name of the application forms here*/
omtr_listofForms="frmstpapply,readycashpersonalloan2,frmnglonhlprod,frmAcquisitionChat,frmtp_reghm,frmngcbolsoecar,frmreadycas,frmsuviocrlp";


/****Other milestones: END****/
...
s.siteID="IN";			// leftmost value in pagename
...
function s_doPlugins(s) {
	...
	s.prop50="Citi Intl IN s_code v1.1 - 20110627 | SiteCatalyst Base Code H23.3";
	...
	/* Internal Campaign & Click-Through */
	if(!s.eVar2) s.eVar2=s.getQueryParam(omtr_internalcampaignID);
	...
	/* External Campaign */
	if(!s.campaign) s.campaign=s.getQueryParam(omtr_externalcampaignID);
	...
	/* UserID */
    s.prop5=s.getAndPersistValue(s.prop5,'s_pers_c5',1825);
    if(!s.prop5){s.prop5="anon";}
	
	/* Client Status */
	s.prop6=s.getAndPersistValue(s.prop6,'s_pers_c6',1825);
